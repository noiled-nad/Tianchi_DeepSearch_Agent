...
4) 集成 ResearchMemory 记录规划步骤（方向三）
"""
...
def make_parse_claims_node(llm) -> Callable[[DeepResearchState], DeepResearchState]:
    async def parse_claims(state: DeepResearchState) -> DeepResearchState:
        ...
        # ── 创建 plan 的 ResearchStep ──
        plan_step = create_step(
            step_type="plan",
            step_id="plan_0",
            input_summary=f"问题: {question[:100]}",
            output_summary=f"拆解为 {len(subtasks)} 个子任务, {len(parallel_groups)} 层并行组",
            success=True,
        )
        plan_step.start_time = step_start
        plan_step.end_time = step_end
        plan_step.duration_ms = (step_end - step_start) * 1000
        plan_step.sub_steps = [st["id"] for st in subtasks]  # 记录子步骤 ID
        memory.add_step(plan_step)
        ...
        print(f"[parse_claims] memory stats: {stats['total_steps']} steps")
