...
async def main():
    ...
    print("\n====== [subtask_findings 输出] ======")
    for st_id, findings_data in (result_state.get("subtask_findings", {}) or {}).items():
        # 兼容新旧格式：findings 可能是字符串或字典
        if isinstance(findings_data, dict):
            findings_text = findings_data.get("findings", "")
            duration = findings_data.get("duration_ms", 0)
            llm_calls = findings_data.get("llm_calls", 0)
            print(f"  [{st_id}] {findings_text[:200]}... (duration={duration:.0f}ms, llm_calls={llm_calls})")
        else:
            print(f"  [{st_id}] {str(findings_data)[:200]}")
